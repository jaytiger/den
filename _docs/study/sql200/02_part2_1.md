...
SELECT COUNT(*), -- * star는 행의 모든 컬럼을 의미합니다.
       COUNT(1), -- 1과 같은 상수를 카운트하면 컬럼 값은 신경 안쓰겠다 입니다. * 와 기본적으로 동일합니다.
       COUNT(empno), -- empno 컬럼 값이 NULL 이 아닌 경우만 카운트 하겠다입니다.
       COUNT(comm), -- comm은 다수의 NULL 을 포함하고 있으므로 테이블의 행의 갯수와 다른 결과가 나옵니다.
```
